...
class kitToolsLibrary {
  ...
  public function getMediaPath() {
  	$result = $this->addSlash(CAT_PATH) . $this->removeLeadingSlash($this->addSlash(MEDIA_DIRECTORY));
  	return $result;
  }
  ...
  public function getFileNameByPageID($pageID, &$fileName) {
    ...
    elseif ($sql_result->numRows() > 0) {
      ...
      if(is_file(CAT_PATH . $settings['pages_directory'] . $thisArr['link'] . $settings['page_extension'])) {
  ...
  public function getUrlByPageID($pageID, &$url, $ignore_topics=false) {
  	...
  	if (defined('TOPIC_ID') && !$ignore_topics) {
  		...
  		if (false !== ($link = $database->get_one($SQL))) {
  			$url = CAT_URL.PAGES_DIRECTORY.'/topics/'.$link.PAGE_EXTENSION;
  		}
  		else {
  			return false;
  		} 		
    ...
    elseif ($this->getFileNameByPageID($pageID, $url)) {
    	$url = CAT_URL.PAGES_DIRECTORY.'/'.$url;
    }
    else {
    	return false;
    }    
  ...
  public function removeDirectory($target) {
	 ...
	 * Expl.: findfile(CAT_PATH, $fileregex='/\.(php|inc)$/')
