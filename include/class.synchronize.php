...
class syncServer {
	...
	public function __construct() {
		$this->backup_path = CAT_PATH.MEDIA_DIRECTORY.'/sync_data/backup/';
  ...
  public function getVersion() {
    ...
    $info_text = file(CAT_PATH.'/modules/'.basename(dirname(__FILE__)).'/info.php');
    if ($info_text == false) {
      return -1; 
    }
    // walk through array
...
class syncClient {
	...
	public function __construct() {
		...
		$this->server_url = (empty($server_url)) ? CAT_URL : $server_url;
	} // __construct()
  ...
  public function action() {
  	...
  	//if (($this->params[self::param_server] == CAT_URL) && (!isset($_SESSION[self::session_server_url]))) {
  	if (($this->server_url == CAT_URL) && (!isset($_SESSION[self::session_server_url]))) {
   ...
   */
	...
	public function updateDownload() {
		...
		if (!file_exists(CAT_PATH.'/media/sync_data/backup')) {
			if (!mkdir(CAT_PATH.'/media/sync_data/backup', 0755, true)) {
				$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(sync_error_mkdir, '/media/sync_data/backup')));
				return false;
			}
		}
		if (!rename($this->temp_path.$request[syncServer::result_archive_file], CAT_PATH.'/media/sync_data/backup/'.$request[syncServer::result_archive_file])) {
	...
	public function updateFinished($job_id) {
		...
		if (false !== ($section = $query->fetchRow(MYSQL_ASSOC))) { 
			...
			if (false !== strpos($content, '[[sync_client]]')) { 
				...
				$content = str_replace(sprintf('[[sync_client?server=%s]]', CAT_URL), sprintf('[[sync_client?server=%s]]', $_SESSION[self::session_server_url]), $content);
			}
