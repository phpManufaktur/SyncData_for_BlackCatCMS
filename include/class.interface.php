...
class syncDataInterface {
	...
	public function __construct() {
		...
		$this->temp_path = CAT_PATH.'/temp/sync_data/';
	...
	public function restoreInfo($backup_archive, &$sync_data_ini) {
  	...
  	if (!file_exists($this->temp_path)) {
  		if (!mkdir($this->temp_path, 0755, true)) {
  			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(sync_error_mkdir, str_replace(CAT_PATH, '', $this->temp_path))));
  			return false;
  		}
  	}
  	...
  	$zip = new PclZip(CAT_PATH.$backup_archive);
  	$list = array();
   ...
   */
	private function restoreTables($job_id) { 
  	...
  	// ZIP initialisieren
  	$zip = new PclZip(CAT_PATH.$job[dbSyncDataJobs::field_archive_file]);
		$running = (empty($job[dbSyncDataJobs::field_next_file])) ? true : false;
